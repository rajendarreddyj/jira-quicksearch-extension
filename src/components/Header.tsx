...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header className="bg-white dark:bg-slate-900 text-slate-800 dark:text-white border-b border-slate-200 dark:border-slate-800 sticky top-0 z-30 shadow-xs dark:shadow-md transition-colors">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <button
            ...
            onClick={() => {
              ...
              fullViewUrl.searchParams.set('view', 'full');
              if ((window as any).chrome?.tabs?.create) {
                (window as any).chrome.tabs.create({ url: fullViewUrl.toString() });
              } else {
                window.open(fullViewUrl.toString(), '_blank');
              }
            }}
          ...
          {onOpenNotifications && (
            <button
              ...
              className="relative p-1.5 bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 rounded-md text-xs transition-colors border border-slate-200 dark:border-slate-700 flex items-center justify-center font-bold"
            >
          ...
          {onOpenShortcutsModal && (
            <button
              ...
              className="p-1.5 bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 rounded-md text-xs transition-colors border border-slate-200 dark:border-slate-700 flex items-center justify-center font-mono font-bold"
            >
      ...
      {/* Navigation Tabs Bar */}
      <nav className="flex items-center justify-between px-1.5 sm:px-2 py-1 bg-white dark:bg-slate-900/90 text-xs overflow-x-hidden gap-1 border-t border-slate-200 dark:border-slate-800">
        <button
        ...
        >
          ...
          {historyCount > 0 && (
            <span className={`text-[10px] px-1.5 py-0.2 rounded-full font-bold ${
              activeTab === 'history' ? 'bg-white/20 text-white' : 'bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-300'
            }`}>
              {historyCount}
            </span>
          )}
        ...
        >
          ...
          {cachedCount > 0 && (
            <span className={`text-[10px] px-1.5 py-0.2 rounded-full font-bold ${
              activeTab === 'cached' ? 'bg-white/20 text-white' : 'bg-white dark:bg-slate-800 text-emerald-700 dark:text-emerald-400'
            }`}>
              {cachedCount}
            </span>
          )}
